...
const AddProduct = () => {
  ...
  /*Added newly,remove if its not working properly later */
    const loadSampleData = () => {
    const sample = sampleProducts[sampleIndex];
    if (sample) {
      setProductName(sample.productName);
      setMrp(sample.price.mrp);
      setCost(sample.price.cost);
      setDiscountPercent(sample.price.discountPercent);
      setSubcategory(sample.subcategory);
      setProductImage(sample.productImage);
  ...
  // Move to next sample and load it
  const nextSample = () => {
    setSampleIndex((prev) => {
      const nextIndex = (prev + 1) % sampleProducts.length;
      return nextIndex;
    });
  };

  // Automatically load sample on sampleIndex change
  useEffect(() => {
    loadSampleData();
  }, [sampleIndex]);
        ...
        >
          <div>
            ...
            <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
              <BlueButton variant="outlined" onClick={loadSampleData}>Load Sample</BlueButton>
              <BlueButton variant="outlined" onClick={nextSample}>Next Sample</BlueButton>
            </Stack>
